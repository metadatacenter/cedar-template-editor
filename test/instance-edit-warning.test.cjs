...
test('modal uses exact copy and explicit Continue editing / Cancel choices', () => {
  ...
  assert.equal(options.text, 'You can change display labels, descriptions, help text, and child field or element order. Other changes will require creation of a new template version on save. If a new template version is required, existing instances will remain attached to the original template.');
