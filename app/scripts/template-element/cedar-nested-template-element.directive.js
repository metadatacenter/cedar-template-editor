      ...
      scope.nestElement = function () {
       ...
       var getId = function (node) {
        ...
        if (getNested(scope.field)) {
          template = '<div ng-if="$root.schemaOf(field)._ui.inputType" > <field-directive nested="true" field="field" model="model"  preview="false" rename-child-key="renameChildKey" is-edit-data="isEditData" ></field-directive></div><div ng-if="!$root.schemaOf(field)._ui.inputType" class="nested-element"><cedar-template-element parent-element="parentElement"   key="key" model="model" element="field" ></cedar-template-element></div>';
        } else {
          template = '<div ng-if="$root.schemaOf(field)._ui.inputType" > <field-directive nested="false" field="field" model="model"  preview="false" rename-child-key="renameChildKey" is-edit-data="isEditData" ></field-directive></div><div ng-if="!$root.schemaOf(field)._ui.inputType" class="nested-element"><cedar-template-element parent-element="parentElement"   key="key" model="model" element="field" ></cedar-template-element></div>';
        }
