...
define([
  ...
  angular.module('cedar.templateEditor.controlTerm.controlTermController', [])
    .controller('ControlTermController', ControlTermController);

  ControlTermController.$inject = ['$rootScope', '$scope', '$element', 'controlTermService', '$q', '$timeout', '$http'];

  /**
   * Controller for the functionality of adding controlled terms to fields and elements.
   */
  function ControlTermController($rootScope, $scope, $element, controlTermService, $q, $timeout, $http) {
      ...
      if (newValue !== undefined) {
        if (properties && properties['@type'] &&
          ...
          for (i = 0; i < properties['@type']['oneOf'][0]['enum'].length; i++) {
            ...
            controlTermService.getClassDetails(classId).then(function(response) {
              if (response) {
                // get ontology details
                acronym = getOntologyAcronym(response);
                controlTermService.getOntologyDetails(acronym).then(function(ontologyResponse) {
    ...
    $scope.controlTerm.fieldSearch = function() {
      ...
      controlTermService.searchClass($scope.controlTerm.fieldSearchTerms).then(function(response) {
    ...
    var loadTreeOfClass = function(selection) {
      ...
      controlTermService.getClassDetails(selfUrl).then(function(response) {
      ...
      $q.all({
        details:    controlTermService.getOntologyDetails(acronym),
        size:       controlTermService.getOntologySize(acronym),
        tree:       controlTermService.getClassTree(acronym, selection['@id']),
        // root:       controlTermService.getOntologyTreeRoot(acronym),
        //valueSet:   controlTermService.getClassValueSet(acronym, selection['@id'])
    ...
    var loadOntologyRootClasses = function(ontology) {
      ...
      $q.all({
        details:    controlTermService.getOntologyDetails(acronym),
        size:       controlTermService.getOntologySize(acronym),
        tree:       controlTermService.getOntologyValueSets(acronym),
        // root:       controlTermService.getOntologyTreeRoot(acronym),
        //valueSet:   controlTermService.getClassValueSet(acronym, selection['@id'])
      }).then(function(values) {
        if($scope.controlTerm.fieldTreeVisibility == true) {
    ...
    $scope.controlTerm.getClassDetails = function(subtree) {
      ...
      controlTermService.getClassDetails(subtree.links.self).then(function(response) {
        $scope.controlTerm.classDetails = response;
      });
    ...
    var loadCategoriesForOntology = function(ontology) {
      controlTermService.getOntologyCategories(ontology.acronym, true).then(function(response) {
    ...
    var loadMetricsForOntology = function(ontology) {
      controlTermService.getOntologySize(ontology.acronym, true).then(function(response) {
        if (!(status in response)) {
    ...
    var loadParentAndDetermineValueSetForValuesSearchResult = function(valueSet) {
      ...
      controlTermService.getClassParents(acronym, valueSet['@id']).then(function(response) {
        if (!(status in response)) {
    ...
    $scope.controlTerm.valuesSearch = function() {
      ...
      if ($scope.controlTerm.bioportalOntologiesFilter && $scope.controlTerm.bioportalValueSetsFilter) {
        controlTermService.searchOntologyClassesValueSetsAndValueSetClasses($scope.controlTerm.valuesSearchTerms).then(function(response) {
          var maxLen = response.collection.length;
        ...
        if ($scope.controlTerm.bioportalOntologiesFilter) {
          controlTermService.searchClass($scope.controlTerm.valuesSearchTerms).then(function(response) {
            var maxLen = response.collection.length;
            if (maxLen > 20) {
              maxLen = 20;
            }
        ...
        // search value sets
          ...
          controlTermService.searchValueSetsAndValues($scope.controlTerm.valuesSearchTerms).then(function(response) {
            var maxLen = response.collection.length;
            if (maxLen > 20) {
              maxLen = 20;
            }
    ...
    $scope.controlTerm.selectValueResult = function(result) {
      ...
      if (result.resultType == 'Ontology' || result.resultType == 'Ontology Class') {
        ...
        if (result.resultType == 'Value Set') {
          ...
          // get the parent
          controlTermService.getClassParents('NLMVS', result['@id']).then(function(response) {
        ...
        controlTermService.getClassValueSet(acronym, $scope.currentValueSetId).then(function(valueSetClasses) {
    ...
    $scope.assignOntologyTree = function(ontologyOrOntologyClass) {
      if (!$scope.controlTerm.selectedValueResult.ontologyTree) {
        ...
        if (ontologyOrOntologyClass["@type"].indexOf("Ontology") >= 0) {
          controlTermService.getOntologyValueSets(ontologyOrOntologyClass.acronym).then(function(response) {
          ...
          controlTermService.getClassTree(acronym, ontologyOrOntologyClass['@id']).then(function(response) {
            response.sort(sortOntologyTree);
            $scope.controlTerm.selectedValueResult.ontologyTree = response;
            $scope.controlTerm.searchPreloader = false;
          });
    ...
    $scope.controlTerm.stageOntologyClassSiblingsValueConstraint = function(selection) {
      $scope.controlTerm.stagedOntologyClassValueConstraints = [];
      controlTermService.getClassParents(getOntologyAcronym(selection), selection['@id']).then(function(response) {
        ...
        if (response && angular.isArray(response) && response.length > 0) {
          controlTermService.getClassChildren(acronym, response[0]['@id']).then(function(childResponse) {
          ...
          controlTermService.getOntologyTreeRoot(acronym).then(function(childResponse) {
            angular.forEach(childResponse, function(child) {
              $scope.controlTerm.stageOntologyClassValueConstraint(child);
            });
            $scope.controlTerm.stageValueConstraintAction = "add_siblings";
