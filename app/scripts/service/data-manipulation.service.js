  ...
  function DataManipulationService(DataTemplateService, DataUtilService, UrlService, FieldTypeService, $rootScope,
    ...
    service.relabelField = function (schema, key, newTitle) {
      ...
      if (properties["@context"] && properties["@context"].properties) {
        ...
        // (Note: we don't do this renaming anymore, so the next lines are commented out. We decided that it's better to
        // use UUIDs for the property URIs generated by CEDAR, so we just keep the URI that was automatically generated.)
        ...
        if (key == k) {
          ...
          if (p["@context"] && p["@context"].properties) {
            ...
            if (prop && prop.enum) {
              if (prop.enum[0].indexOf(UrlService.schemaProperties()) > -1) {
                var randomPropertyName = service.generateGUID();
                prop.enum[0] = service.getEnumOf(randomPropertyName);
              }
            }
          }
          ...
          if (idOfValue && idOfValue == childId) {
            ...
            if (p["@context"] && p["@context"].properties) {
              ...
              if (p["@context"].properties[newKey] && p["@context"].properties[newKey].enum) {
                ...
                p["@context"].properties[newKey].enum[0] = service.getPropertyOf(propertyId, p["@context"].properties[newKey].enum[0]);
              }
            }
            if (p["@context"].required) {
              var idx = p["@context"].required.indexOf(key);
    ...
    service.findChildren = function (iterator, parentModel) {
      ...
      angular.forEach(iterator, function (value, name) {
        ...
        if (name == '@context') {
          ctx = service.generateInstanceContext(value);
        }
      });
