...
define([
  ...
  function cedarBioportalSearch(controlledTermDataService, StringUtilsService) {
    var directive = {
      ...
      controller      : cedarBioportalSearchController,
      controllerAs    : 'cbs',
      restrict        : 'E',
      scope           : {},
    ...
    cedarBioportalSearchController.$inject = ['$q',
                                              '$rootScope',
                                              '$scope',
                                              "controlledTermService", "controlledTermDataService",
                                              "StringUtilsService"];

    function cedarBioportalSearchController($q, $rootScope, $scope, controlledTermService,controlledTermDataService,
                                            StringUtilsService) {
      ...
      function bioportalSearch(query, sources, maxResults, searchClasses, searchValues, searchValueSets,
        ...
        if ((searchClasses) && (!searchValues) && (!searchValueSets)) {
          ...
          return controlledTermDataService.searchClassesAndValues(query, sources, maxResults).then(
              function (response) {
                return response;
              });
        }
