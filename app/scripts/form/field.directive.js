    ...
    function fieldDirective($rootScope, $sce, $document, $translate, SpreadsheetService, DataManipulationService,
        ...
        var linker = function ($scope, $element, attrs) {
            ...
            $scope.isDateField = function() {
                ...
                return $rootScope.schemaOf($scope.field)._ui.inputType == 'date';

            };

            $scope.toggleDateType = function() {

                console.log('toggleDataType');
                var schema = $rootScope.schemaOf($scope.field);
                console.log(schema);

                if (schema._ui.hasOwnProperty('dateType') && schema._ui.dateType == 'date-range') {
                    delete schema._ui.dateType;
                    schema._ui._value = '';

                } else {
                    schema._ui.dateType = 'date-range';
                    schema._ui._value = {'start':'', 'end':''};
                }
            };

            $scope.hasDateRange = function () {
                var inputType = $rootScope.schemaOf($scope.field)._ui.inputType;
                var dateType = $rootScope.schemaOf($scope.field)._ui.dateType;
                return (inputType === "date") && (dateType == "date-range");
            };
            ...
            $scope.$watch("model", function () {
                ...
                $scope.refreshOption = function (index) {
            ...
            $scope.initDateSingle = function () {
                delete $rootScope.schemaOf($scope.field).dateType;
            }
            ...
            $scope.isDateMultiple = function () {
                return $rootScope.schemaOf($scope.field).dateType === 'date-range';
