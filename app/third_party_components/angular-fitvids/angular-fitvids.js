 ...
 *
 * @description
 * Angular direYctive port of FitVids (http://fitvidsjs.com/).  Modified locally, see below.
 */

angular.module('fitVids', []).directive('fitVids', ['$timeout', function ($timeout) {
  'use strict';
  ...
  return {
    ...
    link    : function (scope, element, attr) {
      ...
      // MODIFIED this isn't finding my youtube iframes, so use the getElementsByTagName list instead
      videos = element[0].querySelectorAll(selectors.join(','));
      var list = element[0].getElementsByTagName("iframe");

      // MODIFIED also have to have the timeout or the ng-bind-html from my div around the iframe isn't built yet
      $timeout(function () {
        ...
        angular.forEach(list, function (item) {
          ...
          if (item.src.indexOf('youtube') != -1) {
            ...
            height = (item.tagName.toLowerCase() === 'object' || $item.attr('height')) ? parseInt($item.attr('height'),
                10) : $item.height();
            ...
            $item.wrap('<div class="fluid-width-video-wrapper" />').parent().css('padding-top',
                (aspectRatio * 100) + "%");
